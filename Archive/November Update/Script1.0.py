...
parser.add_argument('-l', '--length', type=int, metavar='', required=True, help='length of consensus motif. ex: 30')
...
#Lets define the 7 user inputted variables first:
            ...
            if beg_index < 0: #in this case, the sequence is likely in the reverse complement
                ...
                if beg_indexrc < 0: #cannot find beginning in either forward or reverse... can we find the ends?
                    ...
                    if end_indexrc < 0: #cannot find reverse end
                        ...
                        #split into repeats by adding a , every repeat unit
                        rx = [tandemrepeat[idx:idx + motif_length] for idx in range(0, len(tandemrepeat), motif_length)]

                        #add another comma to the beginning of r so that it doesnt overwrite the first motif when I turn it into a pd series
                        rx.insert(0, ',')
...
with open("errors_and_frequency.txt", "w") as text_file:
    text_file.write(error)

#save alleles variable as textfile
with open("alleles.txt", "w") as text_file:
